...
- **Playwright FINAL GATE blocked by environment**: Chromium launch fails with `FATAL:base/apple/mach_port_rendezvous_mac.cc:159 Check failed: kr == KERN_SUCCESS` (macOS bootstrap port blocked in this sandbox). 40/42 fail at browser launch, not at assertions. Affects desktop+mobile equally; not a code regression. Re-run gate in an unconfined shell.
